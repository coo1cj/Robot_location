...
df_lidar = pd.read_csv('scenario_4\scan.csv', sep=',')      #### changer le numero de scenario
...
df_encoder = pd.read_csv('scenario_4\sensor_state.csv', sep=',')  #### changer le numero de scenario
r = 0.033
L = 0.08
...
P4 = np.array([[0.98, -0.196], [0.196, 0.98]])  ### rotation initiale scenario_4
...
def draw_env(df, dl, dr):
    ...
    for t, row in df.iterrows():
        ...
        plt.arrow(0, 0, 0.2*np.cos(tt[i]+1.4), 0.2*np.sin(tt[i]+1.4), head_width=0.01, length_includes_head=True, color='red')
        plt.xticks([])
        plt.yticks([])
        plt.draw()
        plt.pause(0.01)
...
#### centrale ineritelle
df_imu = pd.read_csv('scenario_4\imu.csv', sep=',') ### changer le numero de scenario
